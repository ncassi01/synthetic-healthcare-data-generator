"""
Member generator for the synthetic healthcare data generator.

This module provides functionality to generate synthetic member data with high quality,
realistic correlations, and demographic-appropriate health conditions.
"""
...
class MemberGenerator(BaseGenerator):
    """Generator for synthetic member data with high quality and realism."""
    ...
    def __init__(self, config: Dict[str, Any], seed: Optional[int] = None):
        ...
        # Extract member configuration
        self.member_config = config.get('member_config', {})
        self.insurance_config = config.get('insurance_config', {})
        self.clinical_config = config.get('clinical_config', {})
    ...
    def _initialize_name_distributions(self):
        """Initialize more realistic name distributions based on demographics."""
    ...
    def generate(self, count: int) -> List[Member]:
        """
        Generate synthetic member data with high quality and realism.
        ...
        """
        self.logger.info(f"Generating {count} members with high quality and realism")
        members = []
    ...
    def _generate_enhanced_member(self, index: int) -> Member:
        """
        Generate a single synthetic member with high quality and realism.
        
        Args:
            index: Index of the member (used for ID generation).
            
        Returns:
            A synthetic Member object with realistic attributes.
        """
        # Generate member ID
    ...
    def _generate_gender(self) -> str:
        """Generate a random gender based on the configured distribution."""
        gender_dist = self.member_config.get('gender_distribution', {
            'male': 0.48,
            'female': 0.51,
            'other': 0.01
        })
    ...
    def _generate_demographics(self, gender: str) -> Demographics:
        """Generate demographic information for a member."""
        # Generate date of birth based on age distribution
        age_config = self.member_config.get('age_distribution', {
            'min_age': 0,
            'max_age': 100,
            'mean_age': 45,
            'std_dev': 20
        ...
        age = int(np.clip(
            np.random.normal(age_config['mean_age'], age_config['std_dev']),
            age_config['min_age'],
            age_config['max_age']
        ))
        ...
        # Generate other demographic information
        race_options = ['White', 'Black', 'Asian', 'Native American', 'Pacific Islander', 'Other']
        ...
        return Demographics(
            ...
            language='en',
            marital_status=random.choice(marital_status_options) if age >= 18 else None
        )
    
    def _generate_address(self) -> Address:
        """Generate address information for a member."""
        # Get location configuration
        location_config = self.member_config.get('location', {'country': 'US'})
        
        # Generate state based on distribution if available
        state_dist = location_config.get('state_distribution', {})
        if state_dist:
            state = random.choices(
                list(state_dist.keys()),
                weights=list(state_dist.values()),
                k=1
            )[0]
        else:
    ...
    def _generate_contact(self) -> Contact:
        """Generate contact information for a member."""
        return Contact(
    ...
    def _generate_insurance(self, member_id: str) -> Insurance:
        """Generate insurance information for a member."""
        ...
        coverage_dates = self.insurance_config.get('coverage_start_date', {
            'min_year': 2020,
            'max_year': 2025
        })
        
        # Generate plan type and metal level
        plan_type = random.choices(
            list(plan_types.keys()),
            weights=list(plan_types.values()),
            k=1
        )[0]
        
        metal_level = random.choices(
            list(metal_levels.keys()),
            weights=list(metal_levels.values()),
            k=1
        )[0]
        
        # Generate coverage start date
        start_year = random.randint(
            coverage_dates.get('min_year', 2020),
            coverage_dates.get('max_year', 2025)
        )
        start_month = random.randint(1, 12)
        start_day = random.randint(1, 28)  # Avoid invalid dates
        coverage_start_date = date(start_year, start_month, start_day)
        
        # Generate coverage end date (some percentage have no end date)
        coverage_end_date = None
        if random.random() < 0.3:  # 30% chance of having an end date
            end_date = coverage_start_date + timedelta(days=random.randint(180, 365*3))
            if end_date > date.today():
                coverage_end_date = end_date
        
        # Generate copay and coinsurance based on plan type and metal level
        copay = self._generate_copay(plan_type, metal_level)
        coinsurance = self._generate_coinsurance(plan_type, metal_level)
        
        # Generate deductible and out-of-pocket max based on metal level
        deductible, out_of_pocket_max = self._generate_cost_sharing(metal_level)
        
        return Insurance(
            plan_type=plan_type,
            metal_level=metal_level,
            member_id=f"INS{member_id[3:]}",
            group_id=f"GRP{random.randint(10000, 99999)}",
            coverage_start_date=coverage_start_date,
            coverage_end_date=coverage_end_date,
            primary_care_provider_id=f"PCP{random.randint(10000, 99999)}",
    ...
    def _generate_copay(self, plan_type: str, metal_level: str) -> Dict[str, float]:
        """Generate copay amounts based on plan type and metal level."""
        base_copay = {
            'primary_care': 0.0,
            'specialist': 0.0,
            'emergency': 0.0,
            'urgent_care': 0.0,
            'pharmacy_generic': 0.0,
            'pharmacy_brand': 0.0,
            'pharmacy_specialty': 0.0
        }
        
        # HDHP plans typically don't have copays
        if plan_type == 'HDHP':
            return base_copay
        ...
        base_amounts = {
            ...
            'pharmacy_generic': 15,
            'pharmacy_brand': 40,
            'pharmacy_specialty': 100
        }
        
        # Apply multiplier
        for key, value in base_amounts.items():
            base_copay[key] = round(value * multiplier, 2)
        
        return base_copay
    
    def _generate_coinsurance(self, plan_type: str, metal_level: str) -> Dict[str, float]:
        """Generate coinsurance percentages based on plan type and metal level."""
        base_coinsurance = {
            'primary_care': 0.0,
            'specialist': 0.0,
            'emergency': 0.0,
        ...
        # Adjust coinsurance based on metal level
        base_percentage = {
            'Bronze': 0.4,
            'Silver': 0.3,
            'Gold': 0.2,
        ...
        for key in base_coinsurance:
            # HDHP plans typically have higher coinsurance
            if plan_type == 'HDHP':
                base_coinsurance[key] = round(base_percentage + random.uniform(0.05, 0.15), 2)
            else:
                base_coinsurance[key] = round(base_percentage + random.uniform(-0.05, 0.05), 2)
        
        return base_coinsurance
    ...
    def _generate_cost_sharing(self, metal_level: str) -> tuple:
        ...
        # Add some variation
        deductible = base_deductible + random.randint(-500, 500)
        oop_max = base_oop_max + random.randint(-500, 500)
        
        # Ensure oop_max is greater than deductible
        if oop_max <= deductible:
            oop_max = deductible + 1000
        
        return deductible, oop_max
    
    def _generate_conditions(self) -> List[str]:
        """
        Legacy method for generating conditions.
        This is kept for backward compatibility but is no longer used.
        The enhanced version is used instead.
        """
        ...
        return random.sample(common_conditions, min(num_conditions, len(common_conditions)))
    
    def _generate_medications(self) -> List[str]:
        """
        Legacy method for generating medications.
        This is kept for backward compatibility but is no longer used.
        The enhanced version is used instead.
        """
        ...
        common_medications = [
            ...
            "Fluticasone", "Montelukast", "Escitalopram", "Amoxicillin"
        ]
        
        # Determine number of medications
        num_medications = int(np.clip(
            np.random.normal(medication_config['mean'], medication_config['std_dev']),
        ...
        return random.sample(common_medications, min(num_medications, len(common_medications)))
    ...
    def _generate_procedures(self) -> List[str]:
        """
        Legacy method for generating procedures.
        This is kept for backward compatibility but is no longer used.
        The enhanced version is used instead.
        """
        ...
        # Common procedures
        common_procedures = [
            "Annual Physical", "Colonoscopy", "Mammogram", "Flu Vaccination",
            "Blood Test", "X-Ray", "MRI", "CT Scan", "Ultrasound",
            "EKG", "Stress Test", "Endoscopy", "Biopsy", "Cataract Surgery",
            "Joint Injection", "Physical Therapy", "Cardiac Catheterization"
        ]
        
        # Determine number of procedures
        ...
        return random.sample(common_procedures, min(num_procedures, len(common_procedures)))
